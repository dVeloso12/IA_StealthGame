...
public class MazeExtension : MonoBehaviour
{
    //[System.Serializable]
    ...
    IEnumerator CreateWalls()           //Cria as paredes com base nas dimensões dadas pelo utilizador
    {
        ...
        wallHolder.name = "Maze";         //Objeto usado para "limpar" a cena
        initialPos = new Vector3((-xSize / 2) + wallLength / 2, 0.0f, (startPosZ -(startPosZ/2)) + wallLength / 2);  //Esta posição inicial obtem a parede mais "baixa" e mais "á esquerda" através do posição (0,0)
        ...
        for (int i = 0; i < ySize; i++)
        {
            ...
            for (int j = 0; j <= xSize; j++)     //normalmente o loop gera um conjunto de paredes a mais , por isso paramo-lo um numero antes
            {
                ...
                if (i == 8 && j == 0)
                {
                    //Instantiate(trigger, new Vector3(0, 0, myPos.z), Quaternion.identity);
                    Debug.Log("Trigger");
                }
    ...
    void CreateCells()
    {
        ...
        GameObject[] allWalls;
        int children = wallHolder.transform.childCount+xSize;
        ...
        //{
        //    allWalls[i] = edge[edge.Length-xSize-1+i];
        ...
        for (int i = 0; i < children; i++)
        {
            ...
            allWalls[i] = wallHolder.transform.GetChild(i).gameObject;
                ...
                allWalls[i] = wallHolder.transform.GetChild(i-xSize).gameObject;
        }
        ...
        for (int cellprocess = 0; cellprocess < cells.Length; cellprocess++)
        {
            ...
            //if (cellprocess <= xSize)
            //    cells[cellprocess].south = allWalls[cellprocess];
            //else
        ...
        Debug.Log(cells[10].south + " a" + allWalls[120]);
    ...
    void BreakWall()
    {
        switch (wallToBreak)
        {
            ...
            case 4: Destroy(cells[currentCell].south); Debug.Log(currentCell+"South"); break;
        }
    }
